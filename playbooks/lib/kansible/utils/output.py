...
def system(vagrant_version=None):
    # Get most recent KS Docker Symfony CHANGELOG entry
    changelog_msg = ''
    ...
    if os.path.isfile(changelog):
        ...
        if release is not None:
            changelog_msg = '\n  KS Docker Symfony {0}'.format(release.group(1))

        # Retrieve most recent changelog entry
        else:
            change = re.search(r'^\*\s?(\[BREAKING\])?([^\(\n\[]+)', str, re.M|re.I)
            if change is not None:
                changelog_msg = '\n  KS Docker Symfony version (per changelog): "{0}"'.format(change.group(2).strip())
